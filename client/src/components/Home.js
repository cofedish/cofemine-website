...
const Home = () => {
  ...
  useEffect(() => {
    // Устанавливаем громкость и снимаем muted после загрузки
    if (audioRef.current) {
      audioRef.current.volume = 0.1;
      audioRef.current.muted = false;
    }

    // Плавное появление приветственного контента
    const welcomeTimer = setTimeout(() => setWelcomeVisible(true), 100);

    // Обработчик скролла для sticky меню
    const handleScroll = () => {
    ...
    const fetchServerStatus = async () => {
      try {
        ...
        console.error('Ошибка получения статуса сервера:', err);
      }
    };
    fetchServerStatus();
    const statusInterval = setInterval(fetchServerStatus, 5000);

    // Выбор фонового видео по времени суток
